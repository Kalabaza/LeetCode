...
template <typename T>
ListNode<T>* removeNthFromEnd(ListNode<T>* head, int32_t n) {
   // A dummy node is used before the head, this helps when the head is going to be removed
   auto dummy = new ListNode<T>(0, head);
   ...
   ListNode<T> *first = dummy, *second = head;
...
TEST(PROBLEM_19, Example_1) {
   auto input = makeList<int32_t>({1,2,3,4,5});
...
TEST(PROBLEM_19, Example_3) {
   auto input = makeList<int32_t>({1,2});
   constexpr int32_t n = 1;
