...
int32_t maxArea(const std::vector<int32_t>& height) {
	int32_t maximum = 0;
	// Using a left and right pointers at the start and end of the array of the data
	int32_t left = 0, right = static_cast<int32_t>(height.size()) - 1;
	while (left != right) {
		// Save the maximum volume of water so far, to calculate the volume use the lowest height between the left and
		// right pointer with the length between the two pointers.
